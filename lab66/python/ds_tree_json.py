...
@author: g3863
"""
# 參考資料 參數介紹 http://www.taroballz.com/2019/05/15/ML_decision_tree_detail/
# https://scikit-learn.org/stable/auto_examples/tree/plot_unveil_tree_structure.html
from sklearn.tree import DecisionTreeClassifier
from sklearn.model_selection import train_test_split #切分為訓練與測試資料
...
datacsv = pd.read_csv(str(pathlib.Path.cwd())+'\\water_potability.csv')
datacsv = datacsv.fillna(0)
data = pd.DataFrame(datacsv)
#shuffle_data = data.sample(frac=0.15).reset_index(drop=True) 隨機取樣=>無顯著效果
shuffle_data = data[1:300] #按順序取樣
...
#資料索引
index = list(range(shuffle_data.shape[0]))
#建立MinMaxScaler物件
minmax = preprocessing.MinMaxScaler()
# 資料標準化
data_minmax = minmax.fit_transform(shuffle_data)
X = data_minmax[:,1:]
y = data_minmax[:,0]
